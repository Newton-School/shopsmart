    ...
    <div className="bg-[#FBF0E4]/30 min-h-screen pt-32 pb-20 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-12">
          ...
          <div>
            <h1 className="text-4xl md:text-5xl font-bold text-[#003D29]">Exclusive Deals</h1>
            <p className="text-gray-600 mt-2 text-lg">Hurry up! These offers won&apos;t last long.</p>
          </div>
        </div>

        <div className="space-y-12">
          {deals.map((deal, idx) => (
            <div key={idx} className="bg-white rounded-[3rem] p-8 md:p-12 shadow-sm border border-[#003D29]/10 flex flex-col md:flex-row items-center gap-12 group hover:shadow-xl transition-shadow duration-300 relative overflow-hidden">
              ...
              <div className="w-full md:w-1/3 bg-[#F8F9FA] rounded-4xl p-8 flex justify-center items-center aspect-square">
                <img src={deal.image} alt={deal.name} className="w-full h-auto object-contain max-h-75 group-hover:scale-110 transition-transform duration-500 drop-shadow-2xl" />
              </div>
              
              <div className="flex-1 w-full space-y-6">
                <span className="inline-block px-4 py-2 bg-red-100 text-red-600 font-bold rounded-full text-sm tracking-wide uppercase">{deal.discount}</span>
                <h3 className="text-4xl font-bold text-gray-900 mb-2">{deal.name}</h3>
                <p className="text-gray-500 text-lg leading-relaxed max-w-xl">
                  {deal.title} - Get the best audio experience at a fraction of the price. Limited stock available.
                </p>
                <div className="flex items-baseline gap-4 pt-4">
                  <span className="text-3xl font-bold text-[#003D29]">{deal.price}</span>
