  ...
  return (
    <div className="bg-white min-h-screen font-sans w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-20">
        ...
        <div>
          <h2 className="text-3xl font-bold text-gray-900 mb-8">Headphones For You!</h2>
            ...
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="group relative">
                <div className="w-full aspect-4/5 bg-[#F8F9FA] rounded-[30px] overflow-hidden mb-4 relative p-6 flex items-center justify-center">
                  <button className="absolute top-4 right-4 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  ...
                  <img
                    ...
                    alt="Headphones"
                    className="w-full h-auto object-contain group-hover:scale-110 transition-transform duration-500 drop-shadow-xl"
                  />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-start">
                    <h3 className="font-semibold text-gray-900 text-lg">Wireless Headset</h3>
                    <p className="font-bold text-gray-900 text-lg">$199</p>
                  </div>
                  <p className="text-sm text-gray-500">Organic Cotton</p>
                  <div className="flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <svg key={i} width="16" height="16" viewBox="0 0 24 24" fill="#003D29" stroke="none" className="text-[#003D29]">
                        <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                      </svg>
                    ))}
                    <span className="text-sm text-gray-500 ml-1">(121)</span>
                  </div>
                  <button className="w-full py-3 mt-2 border border-black rounded-full text-sm font-semibold hover:bg-black hover:text-white transition-colors">
                    Add to Cart
                  </button>
                </div>
              </div>
