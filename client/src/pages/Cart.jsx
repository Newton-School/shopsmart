  ...
  if (cart.length === 0) {
    return (
      <div className="bg-[#F8F9FA] min-h-screen pt-32 pb-20 px-4 flex flex-col items-center justify-center font-sans">
        ...
        <p className="text-gray-600 mb-8 max-w-md text-center">It looks like you haven&apos;t added anything to your cart yet. Discover our premium audio products.</p>
        <Link to="/" className="bg-[#003D29] text-white px-8 py-4 rounded-full font-semibold hover:bg-green-900 transition-colors shadow-lg shadow-green-900/20">
          Start Shopping
        </Link>
      </div>
    );
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1">
            <div className="space-y-6">
              {cart.map((item) => (
                <div key={item.id} className="bg-white rounded-4xl p-6 flex flex-col sm:flex-row items-center gap-6 shadow-sm border border-gray-100 relative">
                  ...
                  <div className="flex-1 flex flex-col justify-center sm:pr-12 w-full">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <p className="text-xl font-bold text-[#003D29]">${(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                    <p className="text-sm text-gray-500 mb-6">{item.material || "Premium Selection"}</p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4 bg-gray-50 rounded-full px-4 py-2 border border-gray-200">
                        ...
                        <span className="font-semibold text-lg min-w-6 text-center">{item.quantity}</span>
                        <button 
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
